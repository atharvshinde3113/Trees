...
int findMaxDistance(map<TreeNode*,TreeNode*> &mpp,TreeNode* startnode){
    ...
    while(!q.empty()){
        ...
        for (int i = 0; i < size; ++i)
        {
            ...
            if(mpp[node] && !vis[mpp[node]]){ // traversing parent
                flag = 1;
                vis[mpp[node]] = 1;
                q.push(mpp[node]);
            }
...
// does parent mapping and returns the node where the target lies(since node's val is passed,not address)
TreeNode* bfsToMapParents(TreeNode* root, map<TreeNode*,TreeNode*>&mpp, int start){
    ...
    while(!q.empty()){
        ...
        if(current->val == start) res = current; //return the address of the target node from its int value
...
int timeToBurnTree(TreeNode* root, int start){ // here int value of the node was given and not the address
