...
vector<vector<int>> zigzagLevelOrder(TreeNode* root) {
    ...
    while (!q.empty()) {
        ...
        vector<int> level; // Temporary vector to store the values of nodes at the current level
        ...
        if (!ltor) {
            reverse(level.begin(), level.end()); // Reverse the values to achieve right to left direction
        }
